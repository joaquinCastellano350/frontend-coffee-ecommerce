...
export class WishlistService extends BaseHttpService{
  ...
  toggle(productId: string) {
    ...
    if (set.has(productId)){
      ...
      if (this.auth.isLoggedIn()) {this.deleteOne(productId)} 
    }
    else {
      set.add(productId);
      if (this.auth.isLoggedIn()) { this.persistOne(productId) }
    }
  ...
  private async mergeServer() {
   return await this.http.post('/api/wishlist/merge', {productsIds: this._wishlist()}, {withCredentials: true}).toPromise()
  }
  ...
  async syncAfterLogin() {
    await this.mergeServer().finally(
      async () => {
